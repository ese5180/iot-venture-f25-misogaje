				 ...
				 struct mmc5983ma_raw_data *data)
{
	...
	x_raw = ((uint32_t)raw_bytes[0] << 10) |
		((uint32_t)raw_bytes[1] << 2) |
		((raw_bytes[6] >> 6) & 0x03);

	y_raw = ((uint32_t)raw_bytes[2] << 10) |
		((uint32_t)raw_bytes[3] << 2) |
		((raw_bytes[6] >> 4) & 0x03);

	z_raw = ((uint32_t)raw_bytes[4] << 10) |
		((uint32_t)raw_bytes[5] << 2) |
		((raw_bytes[6] >> 2) & 0x03);
