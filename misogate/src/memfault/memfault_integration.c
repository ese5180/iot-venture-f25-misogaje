...
/*
 * Populate magnetometer metrics here, this is called by Memfault
 *
 * Note: If using NCS >= 2.6.0, set
 * CONFIG_MEMFAULT_NCS_IMPLEMENT_METRICS_COLLECTION=n and call
 * memfault_ncs_metrics_collect() here as well.
 */
void memfault_metrics_heartbeat_collect_data(void) {
  /* Update magnetometer metrics */
  memfault_metrics_heartbeat_set_signed(
      MEMFAULT_METRICS_KEY(mag_x_gauss_x10000), s_mag_x);
  memfault_metrics_heartbeat_set_signed(
      MEMFAULT_METRICS_KEY(mag_y_gauss_x10000), s_mag_y);
  memfault_metrics_heartbeat_set_signed(
      MEMFAULT_METRICS_KEY(mag_z_gauss_x10000), s_mag_z);
  memfault_metrics_heartbeat_set_unsigned(
      MEMFAULT_METRICS_KEY(mag_magnitude_x10000), s_mag_magnitude);

  /* Update read counts */
  memfault_metrics_heartbeat_set_unsigned(
      MEMFAULT_METRICS_KEY(mag_read_success_count), s_mag_read_success);
  memfault_metrics_heartbeat_set_unsigned(
      MEMFAULT_METRICS_KEY(mag_read_error_count), s_mag_read_errors);
