...
void main(void) {
  ...
  while (1) {
    ...
    size_t len = packet_build_secure_frame_encmac(NODE_ID, tx_seq, &m, frame,
                                                  sizeof(frame));
    if (len == 0) {
      ...
      if (rc < 0)
        LOG_ERR("lora_send err %d", rc);
      else
        LOG_INF("sent node=%u seq=%u len=%u", NODE_ID, tx_seq, (unsigned)len);
      tx_seq++;
    }
    k_sleep(K_SECONDS(5));
  }
