...
static void clear_fds(void) { fds[0].fd = -1; }
...
void mqtt_evt_handler(struct mqtt_client *const client,
  ...
  case MQTT_EVT_CONNACK:
    ...
    struct mqtt_topic subscribe_topic = {
        .topic = {.utf8 = MISOGATE_SUB, .size = strlen(MISOGATE_SUB)},
  ...
  case MQTT_EVT_PUBLISH: {
    ...
    if (p->message.payload.len > 0) {
      ...
      LOG_INF("Received on topic \"%.*s\"", p->message.topic.topic.size,
...
static void client_init(struct mqtt_client *client) {
  ...
  client->transport.tls.config.peer_verify =
      TLS_PEER_VERIFY_OPTIONAL; // Relaxed for now, strictly should be REQUIRED
  client->transport.tls.config.cipher_list = NULL;
  client->transport.tls.config.sec_tag_list =
      NULL; // We'll add this if we have certs
...
int mqtt_app_connect(void) {
  ...
  LOG_INF("Initiating MQTT connection to %s:%d (attempt %d/%d)", SERVER_HOST,
          SERVER_PORT, connection_retries + 1, MQTT_MAX_RETRIES);
...
bool mqtt_is_connecting(void) { return connecting; }
