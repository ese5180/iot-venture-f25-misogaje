...
void siphash24(uint8_t out[8], const uint8_t *m, size_t len,
  ...
  case 7:
    b |= ((uint64_t)m[len - 7]) << 48; /* fall through */
  case 6:
    b |= ((uint64_t)m[len - 6]) << 40; /* fall through */
  case 5:
    b |= ((uint64_t)m[len - 5]) << 32; /* fall through */
  case 4:
    b |= ((uint64_t)m[len - 4]) << 24; /* fall through */
  case 3:
    b |= ((uint64_t)m[len - 3]) << 16; /* fall through */
  case 2:
    b |= ((uint64_t)m[len - 2]) << 8; /* fall through */
  case 1:
    b |= ((uint64_t)m[len - 1]); /* fall through */
  default:
    break;
  }
