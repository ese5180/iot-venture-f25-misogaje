 ...
 * magnetic dipole field model. Given measurements from 3 three-axis magnetometers,
 * it solves for the magnet position (x, y) in a known plane.
 *
 * The approach:
...
static inline void vec3_sub(struct vec3_f *out, const struct vec3_f *a, const struct vec3_f *b)
{
    out->x = a->x - b->x;
    out->y = a->y - b->y;
    out->z = a->z - b->z;
...
static inline void vec3_i32_to_f(struct vec3_f *out, const struct vec3_i32 *in)
{
    out->x = (float)in->x;
    out->y = (float)in->y;
    out->z = (float)in->z;
                                   ...
                                   struct vec3_f *B_out)
{
    ...
    /* Prevent infinite values if r_cubed is too small (already handled by r_norm check, but safety first) */
    if (r_cubed < 1.0f)
    {
        r_cubed = 1.0f;
    }
                               ...
                               float J_out[3][3])
{
    ...
    /* dB/dx */
    position_compute_dipole_field(magnet_x + eps_pos, magnet_y, M, sensor, &B_plus);
    position_compute_dipole_field(magnet_x - eps_pos, magnet_y, M, sensor, &B_minus);
    ...
    /* dB/dy */
    position_compute_dipole_field(magnet_x, magnet_y + eps_pos, M, sensor, &B_plus);
    position_compute_dipole_field(magnet_x, magnet_y - eps_pos, M, sensor, &B_minus);
    ...
    position_compute_dipole_field(magnet_x, magnet_y, M + eps_M_actual, sensor, &B_plus);
    position_compute_dipole_field(magnet_x, magnet_y, M - eps_M_actual, sensor, &B_minus);
                              ...
                              struct position_estimate *result)
{
    ...
    if (valid_sensors < 2)
    {
        LOG_WRN("Not enough valid sensors for dipole estimation: %d", valid_sensors);
        return false;
    }
    ...
    for (iter = 0; iter < GN_MAX_ITERATIONS; iter++)
    {
        ...
        for (int nid = 1; nid <= MAX_NODES; nid++)
        {
            ...
            position_compute_dipole_field(theta[0], theta[1], theta[2], sensor, &B_model);
        ...
        {
            LOG_DBG("GN converged at iteration %d, pos_change=%.3f", iter, (double)pos_change);
            break;
        }
    ...
    LOG_INF("GN result: x=%.1f y=%.1f M=%.1f err=%.1f iter=%d",
            (double)result->x, (double)result->y, (double)result->M,
            (double)result->error, result->iterations);
 ...
 * It weights each sensor's position by a function of the measured field strength,
 * then computes a weighted average to estimate the magnet position.
 *
 * Algorithm:
                                     ...
                                     float *out_y)
{
    ...
    for (int i = 1; i <= MAX_NODES; i++)
    {
        ...
        LOG_DBG("Sensor %d: B=%.1f weight=%.1f pos=(%.1f, %.1f)",
                i, (double)B_magnitude, (double)weight,
                (double)sensor->x, (double)sensor->y);
    }
                          ...
                          float *out_y)
{
    ...
    if (position_estimate_triangulation(nodes, out_x, out_y))
    {
        ...
        if (calib_count >= 2 && calib_points != NULL)
        {
            ...
            if (position_estimate_lookup(nodes, calib_points, calib_count,
                                         &lookup_x, &lookup_y))
            {
                ...
                *out_y = 0.7f * (*out_y) + 0.3f * lookup_y;
                ...
                LOG_DBG("Blended with lookup: final=(%.1f, %.1f)", (double)*out_x, (double)*out_y);
            }
        }
    ...
    if (calib_count >= 2 && calib_points != NULL)
    {
        return position_estimate_lookup(nodes, calib_points, calib_count, out_x, out_y);
    }
...
void position_init(void)
{
    ...
    LOG_INF("Dipole orientation m_hat=(%.2f, %.2f, %.2f)",
            (double)g_m_hat.mx, (double)g_m_hat.my, (double)g_m_hat.mz);
}
